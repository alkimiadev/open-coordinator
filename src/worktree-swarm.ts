...
import { openSessionsUi } from "./session-helpers";
...
export const swarmWorktrees = async (
  ...
  for (const task of options.tasks) {
    ...
    if (!createResult.ok) {
      rows.push([rawTask, branch, worktreePath, "-", "error"]);
      notes.push(`${branch}: ${firstLine(createResult.error)}`);
      continue;
    }
    ...
    rows.push([rawTask, branch, worktreePath, createResult.data.id, statusParts.join(", ")]);
    createdCount += 1;
  }
