...
test("open-coordinator add writes config when missing", async () => {
  const root = process.cwd();
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "open-coordinator-e2e-"));
...
test("open-coordinator add dry-run does not write config", async () => {
  const root = process.cwd();
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "open-coordinator-e2e-"));
  const configPath = path.join(tempDir, "opencode.json");
  const cliPath = await resolveCliPath(root);
